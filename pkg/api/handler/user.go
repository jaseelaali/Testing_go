...
import (
	...
	services "github.com/jaseelali/go-gin-clean-arch/pkg/usecase/interface"
)
...
func (cr *UserHandler) FindByEmail(c *gin.Context) {
	...
	if email == "" {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "cannot get email",
		})
		return

	}

	user, err := cr.userUseCase.FindByEmail(c.Request.Context(), string(email))
